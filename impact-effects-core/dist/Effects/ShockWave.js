...
class ShockWaveEffects {
    constructor() {
        ...
        this.centered_overpressure_f = op => 0;
        this.overpressure_f = op => 0;
        this.point_assesment = {
            overpressure: 0,
            max_wind_speed: 0
        };
    }
    calc_heff_and_zero_point(variant) {
    ...
    low_overpressure_calc(variant) {
        let hetp = MathExt_1.default.interpolate_by_density(0.19 * variant.diameter ** 0.4 * Math.sin(variant.angle_rad) ** 0.32, 0.39 * variant.velocity ** 0.26 * Math.sin(variant.angle_rad) ** 0.26, variant.density);
        let hetn = MathExt_1.default.interpolate_by_density(0.5 * variant.diameter ** 0.1, 0.23 + 0.25 * this.heff ** 0.37 * Math.sin(variant.angle_rad) ** 0.52, variant.density);
        let a = 0.0048;
        let pre_res = (d) => 1000000.0 * a * (variant.kenergy_kttnt ** (1. / 3.) / d) ** 1.5;
        return (op) => {
    ...
    high_overpressure_calc(variant) {
        let hetl = MathExt_1.default.interpolate_by_density(1.1 * Math.sin(variant.angle_rad) ** -0.34, 0.56 * Math.sin(variant.angle_rad) ** -0.27, variant.density);
        let hetp = MathExt_1.default.interpolate_by_density(1.7 * Math.sin(variant.angle_rad) ** -0.54, 0.84 * Math.sin(variant.angle_rad) ** -1.2, variant.density);
        let hetn = MathExt_1.default.interpolate_by_density(0.65 + 0.064 * Math.sin(variant.angle_rad) ** -2.8, 0.44 + 0.0083 * Math.sin(variant.angle_rad) ** -3.8, variant.density);
        let a = 0.0046 - 0.0004 * Math.log10(variant.kenergy_kttnt);
        let pre_res = (d) => 1000000.0 * a * (variant.kenergy_kttnt ** (1. / 3.) / d) ** 1.4;
        return (op) => {
            let r = Math.sqrt(op.x ** 2 + op.y ** 2);
            let phi = Math.atan2(op.y, op.x);
            let hetpORhetn = 0 <= phi && phi < Math.PI ? hetp : hetn;
    ...
    overpressure_to_kPa(val) {
        return (val) * 101.325;
    }
    overpressure_calc(variant) {
        let res = (op) => 0;
        if (variant.diameter <= 150)
            res = this.low_overpressure_calc(variant);
        else if (variant.diameter < 300) {
            let var_150 = new Variant_1.default("var_150");
            ...
            let var_300 = new Variant_1.default("var_300");
            ...
            res = (op) => MathExt_1.default.interpolate_by(this.low_overpressure_calc(var_150)(op) + 1, var_150.kenergy_kttnt, this.high_overpressure_calc(var_300)(op) + 1, var_300.kenergy_kttnt, variant.kenergy_kttnt) - 1;
        }
        else
    ...
    max_wind_speed_calc(overpressure_atm) {
