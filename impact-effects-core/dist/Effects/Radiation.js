...
class RadiationEffects {
     ...
     */
        ...
        this.centered_irradiation_f = op => 0;
        this.irradiation_f = op => 0;
        this.centered_irradiation_flux_f = op => 0;
        this.irradiation_flux_f = op => 0;
    }
    calc_hrad_and_zero_point(variant) {
        this.hrad = RadiationEffects.hrad_calc(variant);
        this.zero_point = RadiationEffects.zero_point_calc(variant);
    }
    calc_variant(variant) {
    ...
    static hrad_large(variant) {
        ...
        let res = MathExt_1.default.interpolate_by(hrad_large_1000, 1000, hrad_large_3320, 3320, variant.density);
        if (variant.diameter >= 1500 && res > 100)
            res = 100;
        return res;
    }
    static hrad_calc(variant) {
        let res = 0;
    ...
    static trad_300(variant) {
        return 93000 * variant.kenergy_kttnt ** 0.69 * (Math.sin(variant.angle_rad) ** 0.13) / (variant.velocity ** 5.6);
    }
    static trad_1000(variant) {
        return MathExt_1.default.interpolate_by_density(1.1 * variant.kenergy_kttnt ** 0.33 * (Math.sin(variant.angle_rad) ** 1.4), 4.6 * variant.kenergy_kttnt ** 0.25 * (Math.sin(variant.angle_rad) ** 1.2), variant.density);
    }
    static trad_small(variant) {
        if (variant.density < 2630)
            return MathExt_1.default.interpolate_by(0.01 * variant.heff ** 1.77 * variant.kenergy_kttnt ** 0.67 * (Math.sin(variant.angle_rad) ** 0.68 / variant.velocity ** 1.55), 1000, 0.15 * variant.heff ** 1.01 * variant.kenergy_kttnt ** 0.52 * (Math.sin(variant.angle_rad) ** 0.37 / variant.velocity ** 1.36), 2630, variant.density);
        else
            return MathExt_1.default.interpolate_by(0.15 * variant.heff ** 1.01 * variant.kenergy_kttnt ** 0.52 * (Math.sin(variant.angle_rad) ** 0.37 / variant.velocity ** 1.36), 2630, 0.25 * variant.heff ** 0.81 * variant.kenergy_kttnt ** 0.45 * (Math.sin(variant.angle_rad) ** 0.24 / variant.velocity ** 1.18), 3320, variant.density);
    }
    static trad_calc(variant) {
        if (variant.diameter <= 150)
    ...
    static eta_small_calc(variant) {
        ...
        let res = MathExt_1.default.interpolate_by(res_3320, 3320, res_1000, 1000, variant.density);
        if (res >= 100)
            return 100.;
        if (res < 0)
            return 0.;
    ...
    static zero_point_small_calc(variant) {
    ...
    static max_irradiation_energy_calc(variant) {
        return 1.12 * 10 ** -15 * variant.kenergy ** 1.01;
    }
    static max_irradiation_flux_calc(variant) {
        return MathExt_1.default.interpolate_by_density(0.14 * variant.kenergy_kttnt ** 0.71, 0.01 * variant.kenergy_kttnt ** 0.97, variant.density);
    }
    irradiation_small_calc(variant) {
        let cos_a = Math.cos(variant.angle_rad);
        let log_ke_kttnt = Math.log(variant.kenergy_kttnt);
        let elp_1000_2630 = MathExt_1.default.interpolate_by(1. + 0.03 * cos_a ** 0.03 * log_ke_kttnt, 1000, 1. + 0.13 * cos_a ** (4.38 * 10 ** (-9)) * log_ke_kttnt, 2630, variant.density);
        let elp_2630_3320 = MathExt_1.default.interpolate_by(1. + 0.13 * cos_a ** (4.38 * 10 ** (-9)) * log_ke_kttnt, 2630, 1. - 0.07 * cos_a ** 10.98 * log_ke_kttnt, 3320, variant.density);
        let elp = variant.density <= 2630 ? elp_1000_2630 : elp_2630_3320;
        let eln_1000_2630 = MathExt_1.default.interpolate_by(1. + 0.15 * cos_a ** 0.75 * log_ke_kttnt, 1000, 1. + 0.12 * cos_a ** (6.3 * 10 ** (-11)) * log_ke_kttnt, 2630, variant.density);
        let eln_2630_3320 = MathExt_1.default.interpolate_by(1. + 0.12 * cos_a ** (6.3 * 10 ** (-11)) * log_ke_kttnt, 2630, 1. + 0.13 * cos_a ** 0.015 * log_ke_kttnt, 3320, variant.density);
        let eln = variant.density <= 2630 ? eln_1000_2630 : eln_2630_3320;
        elp = 0.2 < elp && elp < 5 ? elp : 1;
        eln = 0.2 < eln && eln < 5 ? eln : 1;
        return (op) => {
            if (Math.sqrt(op.x ** 2 + op.y ** 2) <= 1)
                return this.max_irradiation_energy;
    ...
    irradiation_large_calc(variant) {
    ...
    irradiation_calc(variant) {
        ...
        if (variant.diameter <= 150)
            ...
            return (op) => {
                if (Math.sqrt(op.x ** 2 + op.y ** 2) <= 1)
                    return this.max_irradiation_energy;
                let res = MathExt_1.default.interpolate_by(this.irradiation_small_calc(var_150)(op), var_150.kenergy_kttnt, this.irradiation_large_calc(var_300)(op), var_300.kenergy_kttnt, variant.kenergy_kttnt);
                return res;
            };
        }
        return this.irradiation_large_calc(variant);
    }
    irradiation_flux_calc(variant) {
        let csc = 1 / Math.sin(variant.angle_rad);
        let cos = Math.cos(variant.angle_rad);
        let log10 = Math.log10(variant.kenergy);
        let eta_flux = MathExt_1.default.interpolate_by_density(100 * 353607.27 * variant.velocity ** 1.42 / variant.kenergy ** 0.54 * csc, 100 * 3.44 * 10 ** 6 * variant.velocity ** 1.37 / variant.kenergy ** 0.61 * csc, variant.density);
        let elp = MathExt_1.default.interpolate_by_density(1 - 0.02 * cos ** 0.19 * (-67.21 + log10), 1 - 0.13 * cos ** 0.0004 * (-17.09 + log10), variant.density);
        let eln = MathExt_1.default.interpolate_by_density(1 + 3.73 * cos ** 1.93 * (-15.71 + log10), 1 + 1.08 * cos ** 0.53 * (-15.11 + log10), variant.density);
        if (eln < 0.15 || eln > 3)
            eln = 1;
